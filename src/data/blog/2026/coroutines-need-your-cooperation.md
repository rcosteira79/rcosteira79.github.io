...
Coroutine cancellation works the same way. The runtime doesn't reach in and stop your coroutine — it broadcasts a signal. Whether anyone actually stops depends entirely on whether the code is listening. If it is — great, it sees the signal and unwinds cleanly. If it isn't — if it's heads-down in a tight loop with no reason to check — the signal just sits there, being ignored, while the coroutine keeps running.
